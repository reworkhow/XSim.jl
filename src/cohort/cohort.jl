...
function test(aa::Bool=true)
    print("aa__", aa)
end


function Animal(mySire::Int64, myDam::Int64)
    my = Animal(Array{Chromosome}(undef, common.G.numChrom),
                Array{Chromosome}(undef, common.G.numChrom),
                Array{Float64}(undef, 0), 0, 0, 0,
                Array{Float64, 1}(undef, 0),
                Array{Float64, 1}(undef, 0), -9999.0)
    my.sireID = mySire
    my.damID  = myDam
    my.myID   = common.countId
    common.countId += 1
    for i in 1:common.G.numChrom
        my.genomePat[i] = Chromosome(Array{AlleleIndexType}(undef, 0),
                                     Array{Int64}(undef, 0),
                                     Array{Float64}(undef, 0),
                                     Array{Float64}(undef, 0))
        my.genomeMat[i] = Chromosome(Array{AlleleIndexType}(undef, 0),
                                     Array{Int64}(undef, 0),
                                     Array{Float64}(undef, 0),
                                     Array{Float64}(undef, 0))
    end
